...
const char *logSAFA="S-AFA.log";
const char *logDAM="DAM.log";
const char *logMDJ="MDJ.log";
const char *logFM9="FM9.log";
const char *logCPU="CPU.log";
...
//	nunca va a ser 0
	...
	buffer = malloc(total= (8*4 + pathEscriptorioTam));//le agrego uno mas por cant de lista archivo
	...
	memcpy(buffer + 7*4 ,&(proceso->quantumRestante),4);
	memcpy(buffer + 8*4 , proceso->pathEscriptorio,pathEscriptorioTam) ;
...
int recibirProceso(int socketSender, Proceso * proceso){
	...
	if(flag != -1){
		...
		memcpy(&(proceso->quantumRestante), buffer + 7*4 ,4 );
