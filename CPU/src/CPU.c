...
//	enviar(sockcoordinador,pack);
    ...
    while(1){
    	char* buff = malloc(1);
    	recv(sockSAFA,buff,2,0); // No hacemos nada con el numero
    	free(buff);
...
//			-------------------------------------------------------------------
		...
		else if ((*proc).flagInicializacion == 1) //LLego un DTB ya inicializado (el posta)
		{
			...
			instructionPointer =  (*proc).seEstaCargando; //desde que linea arranco a ejecutar

			DTB = proc;

			list_iterate(DTB->archivosAsociados, &mostrarInfoArchivoAsociado);
...
//			(*DTB). = (*proc).seEstaCargando;
			...
			if(flagError == 1){
				...
				send(sockSAFA,&instructionPointer,sizeof(int),0);  //Mando en que linea tengo q dps retomar
			}

			DTB = NULL;
//			instructionPointer = 0; esta linea no va mas
		}
...
//			}//EJECUCION
			...
			if(recv(sockSAFA,buff,2,0)){
				if(buff[0] == 'P'){
					recibirProceso(sockSAFA,DTB);
					recv(sockSAFA,buff,2,0);
				}
				if(buff[0] == '1'){
				...
				if(buff[0] == 'E'){  //	Mando orden a CPU para que avise a FM9 que libere la memoria utilizada por la ejecución del proceso
					Paquete_DAM * paqueteLiberarMemoriaProceso = inicializarPaqueteDam();
					paqueteLiberarMemoriaProceso->idProcesoSolicitante =DTB->idProceso;
					paqueteLiberarMemoriaProceso->resultado = -1;
					paqueteLiberarMemoriaProceso->tipoOperacion = 50;
					enviarPaqueteDAM(sockFM9, paqueteLiberarMemoriaProceso);
					flagError = 0;
					break;
				}
				if(buff[0] == 'A'){
...
//agregar concentrar
				...
				archivoABuscar = string_new();
				char *nombreArchivoAescribir = string_new();
				char** datos = string_n_split(sentencia, 3, " " );
				string_append(&nombreArchivoAescribir, datos[0]);
				string_append(&archivoABuscar,nombreArchivoAescribir);
				int* flagFM9 = malloc(sizeof(int));
				DatosArchivo *archivoAsociado = (DatosArchivo*) list_find(DTB->archivosAsociados, &archivoAbierto);
				if(archivoAsociado)
				{
					Paquete_DAM *paquete = inicializarPaqueteDam();
					(*paquete).idProcesoSolicitante = (*DTB).idProceso;
					...
					(*paquete).archivoAsociado.numeroPagina = (*archivoAsociado).numeroPagina ;
					(*paquete).archivoAsociado.numeroSegmento = (*archivoAsociado).numeroSegmento ;
					log_info(logger,"numSegmentoAbuscar..  %d", (*archivoAsociado).numeroSegmento);
					string_append(&((*paquete).archivoAsociado.nombre), sentencia);
					enviarPaqueteDAM(sockFM9, paquete);
					recv(sockFM9,flagFM9,sizeof(int),0);
//					recibirPaqueteDAM(sockFM9, paquete);//NO VA ESTA LINEA
...
//					liberarPaqueteDAM(paquete);
			...
			return 0;
				...
				void* buffer = malloc(4);//4 por int
//				memcpy(buffer,&(tam),4) ;
				send(sockSAFA,&tam,4,0);
				free(buffer);
				buffer = malloc(tam);
				memcpy(buffer,sentencia,tam) ;
				send(sockSAFA,buffer,tam,0);
				free(buffer);
//				char* buf = malloc(2);
...
//				}
				...
				if(archivoAsociado)
				{
					...
					string_append(&((*paquete).archivoAsociado.nombre), sentencia);
					int res = -2;
					res = enviarPaqueteDAM(sockDAM, paquete);
					log_info(logger,"resultado de send PACKDAM: %d ",res);
					send(sockSAFA,"b",2,0); // Le aviso al SAFA que bloquee
				}
				else
				{
...
void mostrarInfoArchivoAsociado(DatosArchivo * archivoAsociado){
	...
	log_info(logger,"Numero Segmento: %d",archivoAsociado->numeroSegmento);
	log_info(logger,"Numero Pagina: %d",archivoAsociado->numeroPagina);
