...
//		log_info(logger,"Tipo Operacion:  %d ",(*paquete).tipoOperacion);
//		int sockSAFA=crearConexionCliente(pathSAFA);
//		if(sockSAFA<0 ){
...
//		send(sockSAFA,"2",2,0);
//		int sockMDJ=crearConexionCliente(pathMDJ);
//		if(sockMDJ<0 ){
//			log_error(logger,"Error en la conexion con el cliente");
//			return 1;
//		}
...
//		send(sockMDJ,"2",2,0);
//		int sockFM9=crearConexionCliente(pathFM9);
//		if(sockFM9<0 ){
//			log_error(logger,"Error en la conexion con el cliente");
//			return 1;
//		}
//		log_info(logger,"Se realizo correctamente la conexion con FM9");
//		send(sockFM9,"3",2,0);
...
//     		pthread_mutex_unlock(&mutex_pedido);
	...
	//     				string_append(&(contenido), buffer);
     		...
     		log_info(logger,"Proceso: %d  ,Flag MDJ (rta abrir)..: %d",paquete->idProcesoSolicitante ,flagMDJ);
...
//				}
				...
				enviarPaqueteDAM(sockMDJ, paquete);
				log_info(logger,"Partes: %d", partes);
				log_info(logger,"Enviando orden a MDJ para que guarde en memoria el archivo (via transfer size)");
				send(sockMDJ,&partes,sizeof(int),0);
				int transferSizeRestante;
				while( partes > 0){
					if(partes == 1)
					{
						recv(sockFM9,&transferSizeRestante,sizeof(int),0);
						recv(sockFM9,buffer,transferSizeRestante,0);
						send(sockMDJ,&transferSizeRestante,sizeof(int),0);
						send(sockMDJ,buffer,transferSizeRestante,0);
					}
					else
					{
						recv(sockFM9,buffer,transferSize,0);
	//     				string_append(&(contenido), buffer);
				...
				if(partes == 1){
					paquete->resultado =1;
					log_info(logger,"MDJ pudo guardar el archivo");
				}
				else if(partes == 0){
					paquete->resultado =0;
					log_info(logger,"MDJ no pudo guardar el archivo");
				}
